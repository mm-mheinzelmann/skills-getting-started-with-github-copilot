...
# Guards check-and-modify operations on `activities` since FastAPI runs sync
# handlers in a threadpool, allowing concurrent requests to race.
...
def unregister_from_activity(activity_name: str, email: str):
    ...
    Raises:
        HTTPException(404): if `activity_name` does not exist.
        HTTPException(404): if `email` is not signed up for the activity.
    """
    if activity_name not in activities:
        raise HTTPException(status_code=404, detail="Activity not found")
